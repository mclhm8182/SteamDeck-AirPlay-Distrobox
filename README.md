...
```
> 注：国内网络环境如遇 TLS 握手超时，可将源替换为国内镜像加速，例如 `docker.1panel.live/library/ubuntu:latest`。
...
* **启动投屏**: 切换至游戏模式，运行配置好的 `AirPlay` 快捷方式。等待出现终端黑屏后，在 iOS/iPadOS 控制中心点击“屏幕镜像”并选择 Steam Deck 设备。退出时按 `STEAM 键` 选择退出游戏即可释放所有后台进程。
